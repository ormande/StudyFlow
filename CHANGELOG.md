...
- **Frontend (`.env` local / Vercel):** `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY` e `VITE_SENTRY_DSN` reconfigurados.
- **Supabase Edge Functions → Secrets:** `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SB_SERVICE_ROLE_KEY`, `BREVO_API_KEY`, `TELEGRAM_BOT_TOKEN`, `ADMIN_CHAT_ID` e `FUNCTION_SECRET` documentados e reconfigurados no painel.

### Operações / Infraestrutura
- **Bot Telegram:** Novo token via BotFather (token anterior exposto); `ADMIN_CHAT_ID` obtido via API; `FUNCTION_SECRET` gerado para proteger o webhook da função `telegram-bot`.
- **Brevo:** `BREVO_API_KEY` vinculada à API REST (`api.brevo.com`) para e-mails transacionais pós-compra.

### Pendente
- **Pagamentos:** Perda de acesso à conta Efí Bank — secrets `EFI_CLIENT_ID`, `EFI_CLIENT_SECRET` e `EFI_SANDBOX` **não configurados**; checkout PIX/assinaturas indisponível até nova integração.
- **Próximo passo:** Migração do gateway de **Efí Bank → Asaas** (Edge Functions `efi-*` e fluxo de checkout).
