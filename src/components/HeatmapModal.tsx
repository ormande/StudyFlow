...
// Função auxiliar para formatar data em YYYY-MM-DD
...
const calculateStats = (logs: StudyLog[], days: number) => {
  ...
  for (let i = 0; i < daysArray.length; i++) {
    const dateStr = formatDateString(daysArray[i]);
    const hasStudy =
      groupedByDate[dateStr] && groupedByDate[dateStr].length > 0;
