...
export function useGoals(logs: StudyLog[]) {
  ...
  useEffect(() => {
    const loadGoals = () => {
      try {
        ...
        const dailyQuestions = localStorage.getItem(
          "studyflow_daily_questions_goal"
        );
        ...
        const loadedGoals: Goals = {
          dailyTimeGoal: dailyTime
            ? parseFloat(dailyTime)
            : DEFAULT_GOALS.dailyTimeGoal,
          weeklyTimeGoal: weeklyTime
            ? parseFloat(weeklyTime)
            : DEFAULT_GOALS.weeklyTimeGoal,
          dailyQuestionsGoal: dailyQuestions
            ? parseInt(dailyQuestions)
            : DEFAULT_GOALS.dailyQuestionsGoal,
    ...
    (newGoals: Goals) => {
      try {
        localStorage.setItem(
          "studyflow_daily_time_goal",
          newGoals.dailyTimeGoal.toString()
        );
        localStorage.setItem(
          "studyflow_weekly_time_goal",
          newGoals.weeklyTimeGoal.toString()
        );
        localStorage.setItem(
          "studyflow_daily_questions_goal",
          newGoals.dailyQuestionsGoal.toString()
        );
        localStorage.setItem(
          "studyflow_weekly_questions_goal",
        ...
        localStorage.setItem(
          ...
          JSON.stringify(newGoals.settings)
        );
        setGoals(newGoals);
  ...
  const getDailyProgress = useCallback(() => {
    ...
    // Questões corretas
    const totalQuestions = todayLogs.reduce(
      ...
      time: {
        ...
        percentage:
          goals.dailyTimeGoal > 0
            ? Math.min((totalHours / goals.dailyTimeGoal) * 100, 200)
            : 0,
      },
      questions: {
        ...
        percentage:
          goals.dailyQuestionsGoal > 0
            ? Math.min((totalQuestions / goals.dailyQuestionsGoal) * 100, 200)
            : 0,
      },
    };
  }, [logs, goals]);
  ...
  const getWeeklyProgress = useCallback(() => {
    ...
    // Questões corretas
    const totalQuestions = weekLogs.reduce(
      (sum, log) => sum + (log.correct || 0),
      0
    );
      ...
      time: {
        ...
        percentage:
          goals.weeklyTimeGoal > 0
            ? Math.min((totalHours / goals.weeklyTimeGoal) * 100, 200)
            : 0,
      },
      questions: {
        current: totalQuestions,
        ...
        percentage:
          goals.weeklyQuestionsGoal > 0
            ? Math.min((totalQuestions / goals.weeklyQuestionsGoal) * 100, 200)
            : 0,
      },
    };
  }, [logs, goals]);
