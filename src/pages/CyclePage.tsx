              ...
              {subject.subtopics.map((subtopic, subtopicIndex) => (
                ...
                >
                  <div className="flex flex-col gap-0.5 opacity-50 hover:opacity-100 transition-opacity">
                    <Button
                      onClick={() =>
                        moveSubtopic(subject.id, subtopicIndex, "up")
                      }
                    ...
                    <Button
                      onClick={() =>
                        moveSubtopic(subject.id, subtopicIndex, "down")
                      }
                  ...
                  <button
                    ...
                    aria-label={
                      subtopic.completed
                        ? `Marcar subtópico "${subtopic.name}" como não concluído`
                        : `Marcar subtópico "${subtopic.name}" como concluído`
                    }
                    title={
                      subtopic.completed
...
export default function CyclePage({
  ...
  const handleSaveEdit = () => {
    ...
    const previousSubject = cycleSubjects.find(
    ...
    setCycleSubjects((prev) =>
      prev.map((s) => (s.id === editingSubject.id ? { ...s, ...updates } : s))
    );
    ...
    Promise.resolve(onUpdateSubject(editingSubject.id, updates)).catch(() => {
      // Reverter em caso de erro
      setCycleSubjects((prev) =>
        prev.map((s) => (s.id === editingSubject.id ? previousSubject : s))
      );
  ...
  const handleDeleteSubtopic = (subjectId: string, subtopicId: string) => {
    ...
    const updatedSubtopics = subject.subtopics.filter(
      (st) => st.id !== subtopicId
    );
  ...
  const moveSubtopic = (
    ...
    if (direction === "up" && subtopicIndex > 0) {
      [newSubtopics[subtopicIndex], newSubtopics[subtopicIndex - 1]] = [
        newSubtopics[subtopicIndex - 1],
        newSubtopics[subtopicIndex],
      ];
    } else if (
      direction === "down" &&
      subtopicIndex < newSubtopics.length - 1
    ) {
      [newSubtopics[subtopicIndex], newSubtopics[subtopicIndex + 1]] = [
        newSubtopics[subtopicIndex + 1],
        newSubtopics[subtopicIndex],
    ...
    <div className="max-w-lg md:max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
              ...
              <Reorder.Group
                ...
                onReorder={(newOrder) => {
                  ...
                  Promise.resolve(onReorderSubjects(newOrder)).catch(() => {
                    ...
                    addToast(
                      "Erro ao reordenar matérias. Tente novamente.",
                      "error"
                    );
                  });
                }}
      ...
      <ConfirmModal
        ...
        message={`Tem certeza que deseja excluir "${
          cycleSubjects.find((s) => s.id === deleteSubjectId)?.name
        }"?`}
        confirmText="Excluir"
        cancelText="Cancelar"
        variant="danger"
        onConfirm={() => {
          if (deleteSubjectId) {
            // ✅ OPTIMISTIC UI: Salvar estado anterior para possível reversão
            const subjectToDelete = cycleSubjects.find(
            ...
            // Atualizar UI imediatamente
            setCycleSubjects((prev) =>
              prev.filter((s) => s.id !== deleteSubjectId)
            );
            setDeleteSubjectId(null);
            ...
            Promise.resolve(onDeleteSubject(deleteSubjectId)).catch(() => {
              ...
              const originalIndex = subjects.findIndex(
                (s) => s.id === deleteSubjectId
              );
              setCycleSubjects((prev) => {
