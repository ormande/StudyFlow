...
export default function CyclePage({
  ...
  const handleAddSubtopic = (subjectId: string) => {
    ...
    onUpdateSubject(subjectId, {
      subtopics: [...subject.subtopics, subtopic],
    });
  ...
  return (
    <div className="max-w-lg mx-auto px-6 py-6 pb-24">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">Ciclo de Estudos</h1>
        <p className="text-gray-600 text-sm">Gerencie suas matérias e acompanhe o progresso</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-lg mb-8">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800">Status do Ciclo</h2>
            <p className="text-xs text-gray-400">Média geral de conclusão</p>
          </div>
          {/* REMOVIDO O QUADRADO CINZA EM VOLTA DO ÍCONE, AGORA É SÓ O ÍCONE */}
        ...
        {/* BOTÃO SÓLIDO LARANJA */}
        <button 
          onClick={onRestartCycle}
          className="w-full py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold shadow-md transition-transform active:scale-95 flex items-center justify-center gap-2"
        >
          <RefreshCw size={18} />
          ENCERRAR E REINICIAR CICLO
        </button>
      </div>
      ...
      <div className="space-y-4 mb-12">
        {subjects.map((subject) => {
          ...
          return (
            <div key={subject.id} className="bg-white rounded-2xl shadow-md overflow-hidden">
              <div className="p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Deseja realmente excluir a matéria "${subject.name}"?`)) {
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between py-2 text-sm font-semibold text-emerald-600 hover:text-emerald-700"
                >
                  <span>
                    Subtópicos ({subject.subtopics.filter((st) => st.completed).length}/
                ...
                <div className="px-5 pb-5 border-t border-gray-100 pt-4">
                  <div className="space-y-2 mb-4">
                    {subject.subtopics.map((subtopic) => (
                      <div key={subtopic.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                        <button
                        ...
                        <span className={`flex-1 text-sm ${subtopic.completed ? 'text-gray-500 line-through' : 'text-gray-800 font-medium'}`}>
                          {subtopic.name}
                        </span>
                        <button
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') handleAddSubtopic(subject.id);
                      }}
                    />
                    <button
                      onClick={() => handleAddSubtopic(subject.id)}
                      className="px-4 py-2.5 rounded-xl bg-emerald-500 text-white hover:bg-emerald-600 transition-all"
                    >
        ...
        {!isAdding ? (
          <button
            ...
            className="w-full py-5 rounded-2xl border-2 border-dashed border-gray-300 text-gray-600 font-semibold hover:border-emerald-500 hover:text-emerald-600 transition-all flex items-center justify-center gap-2"
          >
            <Plus className="w-5 h-5" />
            Adicionar Matéria
          </button>
          ...
          <div className="bg-white rounded-2xl shadow-md p-5 space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Meta do Ciclo (minutos)</label>
              <input
                type="number"
                min="1"
                value={newGoal}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-gray-200 text-gray-700 font-semibold hover:bg-gray-300 transition-all"
              >
                Cancelar
              </button>
            </div>
